        ...
        async with PDFPlaywrightComputer() as computer:
            try:
                ...
                for page_data in pages_data:
                    ...
                    logger.info(
                        f"Screenshot base64 length: {len(screenshot_base64)} chars"
                    )
                    ...
                    try:
                        ...
                        if len(screenshot_bytes) < 100:
                            logger.warning(
                                f"Screenshot for page {page_num} seems too small ({len(screenshot_bytes)} bytes)"
                            )
...
Return your analysis as a concise list of specific issues with exact text quotes and LaTeX fixes.""",
                        ...
                        logger.info(
                            f"Page {page_num} analysis completed with Computer Use"
                        )
                        ...
                        try:
                            ...
                            issue_count = (
                                analysis_lower.count("issue:")
                                + analysis_lower.count("affected text:")
                                + analysis_lower.count("fix:")
                            )
                            fix_count = (
                                analysis_lower.count("latex fix:")
                                + analysis_lower.count("fix command:")
                                + analysis_lower.count("latex:")
                            )

                            # Log detailed metadata for this page
                            logger.info(f"Page {page_num} metadata:")
                            logger.info(
                                f"  - Screenshot size: {len(screenshot_bytes)} bytes"
                            )
                            logger.info(
                                f"  - Base64 encoding size: {len(screenshot_base64)} characters"
                            )
                            logger.info(
                                f"  - Analysis response length: {len(analysis)} characters"
                            )
                            ...
                            if "issue:" in analysis_lower:
                                ...
                                for line in lines:
                                    if (
                                        "issue:" in line.lower()
                                        or "affected text:" in line.lower()
                                    ):
                                        issues.append(line.strip())

                                if issues:
                                    logger.info(
                                        f"  - Specific issues found on page {page_num}:"
                                    )
                            ...
                            logger.warning(
                                f"Could not extract analysis metadata for page {page_num}: {meta_e}"
                            )

                    except Exception as e:
                        logger.error(
                            f"Error analyzing page {page_num} with Computer Use: {e}"
                        )
                        logger.error(
                            f"Screenshot info - bytes: {len(screenshot_bytes)}, base64: {len(screenshot_base64)}, path: {image_path}"
                        )
                        import traceback
        ...
        capture_time = time.time() - start_time
        logger.info(
            f"Completed Computer Use PDF analysis: {len(page_analyses)} pages in {capture_time:.2f}s"
        )
        logger.info(f"Screenshots saved to: {screenshot_paths}")

        # Process results using the same extraction logic as the original tool
        def extract_issues_from_analysis(
            analysis_text: str, page_num: int
        ) -> list[str]:
            ...
            for line in lines:
                ...
                if (
                    (
                        line.startswith("###")
                        and any(char.isdigit() for char in line[:20])
                    )
                    or (line.startswith("Issue") and ":" in line[:20])
                    or (line.startswith(f"{page_num}.") and len(line) < 100)
                ):
        ...
        for page_analysis in page_analyses:
            if page_analysis["analysis"] and not page_analysis["analysis"].startswith(
                "Analysis failed"
            ):
                # Extract individual issues from this page's analysis
                page_issues = extract_issues_from_analysis(
                    page_analysis["analysis"], page_analysis["page_number"]
                )
