...
class LaTeXExpert:
    ...
    def __init__(self, config: ServerConfig):
        ...
        self._fixing_agent = ErrorFixingAgent(
            diagnostics=self._compilation_diagnostics, config=config
        )
    ...
    async def compile_from_request(
        ...
        try:
            ...
            # Use agents for LaTeX compilation
            logger.info("Using agents for LaTeX compilation")
            ...
            metadata = {
                ...
                "compilation_method": "agent" if request.use_agent else "direct",
            }
    ...
    async def _orchestrate_compilation(
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                compilation_result = await self._compilation_agent.compile_with_agent(
                ...
                if compilation_result.success:
                    ...
                    if (
                        hasattr(compilation_result, "output_path")
                        and compilation_result.output_path
                    ):
                        actual_output_path = Path(compilation_result.output_path)
                        logger.info(
    ...
    def compile_latex_file(
        ...
        """Synchronous wrapper for LaTeX compilation using intelligent agents.

        This is a convenience method for CLI and other synchronous contexts
        that need to compile LaTeX files without dealing with async/await.
        ...
        try:
            ...
            return loop.run_until_complete(self.compile_from_request(request))
